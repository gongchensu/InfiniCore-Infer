#include "jiuge_glm4_impl.hpp"
#include "jiuge_glm4_weight.hpp"
...
void createDeviceResource(Glm4DeviceResource *rsrc, const Glm4Meta *meta,
                          const Glm4Weights *weights,
    ...
    std::vector<std::shared_ptr<Tensor>> w_attn_norm, w_attn_qkv, b_attn_qkv, w_attn_out,
        w_ffn_norm, w_ffn_gate_up, w_ffn_down, w_self_attn_norm, w_post_ffn_norm;
    for (size_t layer = 0; layer < meta->nlayer; layer++) {
        ...
        w_self_attn_norm.push_back(getSelfAttnNorm(meta,weights,layer));
        ...
        w_post_ffn_norm.push_back(getPostFFNNorm(meta, weights, layer));
    }

    auto memory_pool = std::make_shared<MemoryPool>(128 * 1024 * 1024);

    *rsrc = Glm4DeviceResource{
        device,
        dev_id,
        handle,
...
void releaseDeviceResource(Glm4DeviceResource &res) {
    ...
    res.w_self_attn_norm.clear();
    for(auto &t :res.w_self_attn_norm){
        t.reset();
    }
    ...
    res.w_post_ffn_norm.clear();
    for (auto &t : res.w_post_ffn_norm) {
        t.reset();
    }
...
void inferDeviceBatch(const Glm4Meta &meta, Glm4DeviceResource &rsrc,
    ...
    auto logits_out_buf = Tensor::buffer(dt_logits, {ntok, d}, rsrc.memory_pool);
...
__C void
inferBatchGlm4(struct Glm4Model *model,
           const uint32_t *tokens, uint32_t ntok,
           const uint32_t *req_lens, uint32_t nreq, const uint32_t *req_pos,
           struct KVCache **kv_caches,
           const float *temperature, const uint32_t *topk, const float *topp,
...
__C void
forwardBatchGlm4(struct Glm4Model *model,
             const uint32_t *tokens, uint32_t ntok,
             const uint32_t *req_lens, uint32_t nreq, const uint32_t *req_pos,
             struct KVCache **kv_caches,
             void *logits) {
...
void launchDevice(const Glm4Meta &meta, const Glm4Weights *weights, Glm4DeviceResource *rsrc, InferState &state, InferRequest &req,
...
Glm4Model::Glm4Model(const Glm4Meta *_meta, const Glm4Weights *weights, infiniDevice_t device_, std::vector<int> device_ids) : meta(*_meta) {
    int ndev = int(device_ids.size());
    device = device_;
    dev_ids = device_ids;
    dev_resources = std::vector<Glm4DeviceResource>(ndev);
...
__C struct Glm4Model *
createGlm4Model(const Glm4Meta *meta,
                 const Glm4Weights *weights,
    ...
    Glm4Model *model = new Glm4Model(meta, weights, device, device_ids);
    return model;
}

__C void destroyGlm4Model(struct Glm4Model *model) {
