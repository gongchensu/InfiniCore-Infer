#ifndef GLM4_WEIGHT_HPP
#define GLM4_WEIGHT_HPP
...
inline std::shared_ptr<Tensor> getAttnO(Glm4Meta const *meta,
                                        Glm4Weights const *w, size_t layer,
...
inline std::shared_ptr<Tensor> getSelfAttnNorm(
    Glm4Meta const *meta,
    Glm4Weights const *w,
    size_t layer) {
    auto shape = std::vector<size_t>({meta->d});
    return Tensor::weight((char *)(w->self_attn_norm[layer]), w->dt_norm, shape);
}
...
inline std::shared_ptr<Tensor> getPostFFNNorm(
    Glm4Meta const *meta,
    Glm4Weights const *w,
    size_t layer) {
    auto shape = std::vector<size_t>({meta->d});
    return Tensor::weight((char *)(w->post_ffn_norm[layer]), w->dt_norm, shape);
}


inline std::shared_ptr<Tensor> getSinTable(Glm4Meta const *meta) {
...
inline std::shared_ptr<Tensor> getCosTable(Glm4Meta const *meta) {
    auto half_dh = meta->dh / 2;
    auto unit = dsize(meta->dt_logits);
    void *table = std::malloc(meta->dctx * half_dh * unit);
