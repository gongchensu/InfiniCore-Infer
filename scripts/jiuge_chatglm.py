...
class LlamaWeightsNaming:
    ...
    def self_attn_norm(self,i):
        return f"model.layers.{i}.post_self_attn_layernorm.weight"
    ...
    def mlp_norm(self,i):
        return f"model.layers.{i}.post_mlp_layernorm.weight"
...
def convert_glm4_to_llama2_naming(chatglm2_state_dict, config):
    ...
    for key, value in chatglm2_state_dict.items():
        ...
        if 'self_attn' in key:
            ...
            if 'gate_up_proj' in key:
                ...
                old_key = key
                gate_key = old_key.replace('mlp.gate_up_proj', 'mlp.gate_proj')
                up_key = old_key.replace('mlp.gate_up_proj', 'mlp.up_proj')
                if 'weight' in key:
                    new_state_dict[gate_key] = value[:intermediate_size,:]
                    new_state_dict[up_key] = value[intermediate_size:,:]
                elif 'bias' in key:
                    new_state_dict[gate_key] = value[:intermediate_size]
                    new_state_dict[up_key] = value[intermediate_size:]
        ...
        elif 'post_self_attn_layernorm' in key:
            new_key = key  # 待添加
...
class Glm4MetaFromLlama(Glm4MetaCStruct):
    def __init__(self, config, dtype=torch.float16, max_tokens=None):
        ...
        super().__init__(
            ...
            dctx=(
                config[("max_position_embeddings" if"max_position_embeddings" in config else "seq_length" )]
                if max_tokens is None else max_tokens
            ),
            dvoc=config["vocab_size" if "vocab_size" in config else "padded_vocab_size"],
            epsilon=config["rms_norm_eps"] if "rms_norm_eps" in config else 1e-6 ,
...
class Glm4WeightsImpl(Glm4WeightsCStruct):
    def __init__(
        self,
        meta,
        naming,
        ...
        self.self_attn_norm_tensors = [
            state_dict[naming.self_attn_norm(i)].to(torch_dt_norm) for i in range(nlayer)
        ]
        self.self_attn_norm_ptrs = [
            self.self_attn_norm_tensors[i].data_ptr() for i in range(nlayer)
        ]
        self.self_attn_norm = (c_void_p * nlayer)(*self.self_attn_norm_ptrs)
        ...
        self.mlp_norm_tensors = [
            state_dict[naming.mlp_norm(i)].to(torch_dt_norm) for i in range(nlayer)
        ]
        self.mlp_norm_ptrs = [
            self.mlp_norm_tensors[i].data_ptr() for i in range(nlayer)
        ]
        self.post_ffn_norm = (c_void_p * nlayer)(*self.mlp_norm_ptrs)
        

class Glm4BatchedTask:
...
class Glm4ForCauslLM:
    def __init__(
        ...
        if "llama" == config["model_type"]:
            ...
            self.meta = Glm4MetaFromLlama(config, max_tokens=max_tokens)
            self.tokenizer = transformers.AutoTokenizer.from_pretrained(model_dir_path)
            self.weights = Glm4WeightsImpl(
                self.meta,
                LlamaWeightsNaming(),
                model.state_dict(),
                ndev=ndev,
    ...
    def batch_infer_one_round(self, tasks: List[InferTask]):
        ...
        batch_inputs = Glm4BatchedTask(tasks)
    ...
    def generate(self, input_content, max_steps, topp_=1.0, topk_=1, temperature_=1.0):
        ...
        for step_i in range(max_steps):
            ...
            steps += 1
            output_str = (
                # self.tokenizer.tokenizer.id_to_token(output_tokens[0])
                # .replace("▁", " ")
                # .replace("<0x0A>", "\n")
                
                # chatglm2
                self.tokenizer.decode(output_tokens[0])
                # .replace("▁", " ")
                # .replace("<0x0A>", "\n")
            )
    ...
    def perplexity(self, test_sequences: List[Sequence[int]], batch_size=10):
        ...
        for i in range(0, len(test_sequences), batch_size):
            ...
            batch_inputs = Glm4BatchedTask(tasks[:batch_id])
            logits = torch.zeros(
