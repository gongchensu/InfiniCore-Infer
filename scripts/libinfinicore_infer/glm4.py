...
class Glm4MetaCStruct(Structure):
    _fields_ = [
        ("dt_logits", DataType),
        ("nlayer", c_size_t),
        ("d", c_size_t),
...
class Glm4WeightsCStruct(Structure):
    _fields_ = [
        ("nlayer", c_size_t),
        ("dt_norm", DataType),
        ("dt_mat", DataType),
        ...
        ("self_attn_norm", POINTER(c_void_p)),
        ("ffn_norm", POINTER(c_void_p)),
        ("ffn_gate_up", POINTER(c_void_p)),
        ("ffn_down", POINTER(c_void_p)),
        ("post_ffn_norm", POINTER(c_void_p))
    ]


class Glm4ModelCStruct(Structure):
    pass


@register_model
class Glm4Model(BaseModel):
    @classmethod
    def register_lib(cls, lib):
        lib.createGlm4Model.restype = POINTER(Glm4ModelCStruct)
        lib.createGlm4Model.argtypes = [
            POINTER(Glm4MetaCStruct),
            POINTER(Glm4WeightsCStruct),
            DeviceType,
            c_int,
            POINTER(c_int),
        ]

        lib.destroyGlm4Model.argtypes = [POINTER(Glm4ModelCStruct)]
        ...
        lib.inferBatchGlm4.argtypes = [
            POINTER(Glm4ModelCStruct),
            POINTER(c_uint),
            c_uint,
            POINTER(c_uint),
            c_uint,
    ...
    def create_model(self, meta, weights, device_type, ndev, dev_ids):
        return self.lib.createGlm4Model(meta, weights, device_type, ndev, dev_ids)

    def destroy_model(self, model):
        self.lib.destroyGlm4Model(model)
